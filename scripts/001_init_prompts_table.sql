-- Create the analysis_prompts table if it doesn't exist
...
-- Insert the default prompt if the table is empty
INSERT INTO analysis_prompts (prompt_text)
SELECT 'Você é um auditor especializado em análise de conversas entre agentes de IA e usuários. Sua missão é avaliar a qualidade das respostas do agente de IA com base em critérios específicos.
...
```'
WHERE NOT EXISTS (SELECT 1 FROM analysis_prompts LIMIT 1);
